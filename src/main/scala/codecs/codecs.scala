...
trait DecoderInstances:
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromFunction {
      (json: Json) => {
        val ret: List[Option[A]] = json match {
          case Json.Arr(l) => for el <- l yield decoder.decode(el)
        }
        Some(ret.filterNot(_ == None).map(_.get))
      }
    }
...
trait PersonCodecs:
  ...
  given Decoder[Person] =
    Decoder.field[String]("name")
      ...
      .transform[Person]((name: String, age: Int) => Person(name, age))
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] =
    Decoder.field[List[Person]]("people")
      .transform[Contacts]((l: List[Person]) => Contacts(l))
